  ...
  gameScene: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      inintView: function inintView() {
        ...
        for (var i = 0; i < myHandCard.children.length; ++i) {
          var sprite = myHandCard.children[i].getComponent(cc.Sprite);
  ...
  mainScene: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      allBtnCallback: function allBtnCallback(event, name) {
        cc.log(name);
        var self = this;
        "creatroom" === name ? this.create_room_layer.active = true : "joinroom" === name ? this.join_room_layer.active = true : "shop" === name || "diamond" == name ? this.shop_layer.active = true : "rule" === name ? this.rule_layer.active = true : "set" === name || "record" === name || "share" === name;
      },
      initAllBtn: function initAllBtn() {
        cc.log("btn===init");
        var self = this;
...
}, {}, [ "init", "layer_scale_show", "https", "notice", "adaptive", "imgHead", "copy_h5", "err", "event", "funcs", "uiFuncs", "gameScene", "loginScene", "createLayer", "joinLayer", "mainScene", "ruleLayer", "shopLayer", "update" ]);
