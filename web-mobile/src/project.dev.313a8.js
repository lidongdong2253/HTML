window.__require = function e(t, n, r) {
  ...
  adaptiveView: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      judgeView: function judgeView() {
        ...
        if (height > width) {
          config.canvasWidth = cc.view.getCanvasSize().height;
          console.log("config.canvasWidth==", config.canvasWidth);
          return "portrait";
        }
        config.canvasWidth = cc.view.getCanvasSize().width;
        console.log("config.canvasWidth==", config.canvasWidth);
        return "landscape";
      },
      start: function start() {}
    ...
    cc.Class({
      ...
      onLoad: function onLoad() {
        ...
        if (cc.sys.isBrowser) {
          ...
          console.log("cc.visibleRect.width==", cc.visibleRect.width);
          console.log("cc.visibleRect.height==", cc.visibleRect.height);
    ...
    cc.Class({
      ...
      pushRoundResult: function pushRoundResult(data) {
        ...
        if (data.reconnect) self.hideAllHandCard(); else {
          ...
          for (var i = 0; i < wsft.gameData.seats.length; ++i) i == wsft.gameData.mySeatIndex - 1 ? this.initMyHandCard(data.rs[i].mj, true) : this.initOtherHandCard(i + 1);
          setTimeout(function() {
      ...
      initMyHandCard: function initMyHandCard(_HandCard, _boolover) {
        var seats = wsft.gameData.seats;
        var mySeat = seats[wsft.gameData.mySeatIndex - 1];
        var _myhandCard = _HandCard || mySeat.handCard;
        var _boolOver = _boolover || false;
        var myHandCard = _HandCard;
        _boolover || (myHandCard = this.sortHandCard(_myhandCard));
        ...
        for (var i = 0; i < myHandCard.length; ++i) {
          ...
          sprite.node.y = 0;
          if (_boolOver) {
            sprite.node.height = 113;
            sprite.node.width = 75;
            this.setSpriteFrameByMJ("B_", sprite, mjNum, mjType);
          } else this.setSpriteFrameByMJ("M_", sprite, mjNum, mjType);
        }
  ...
  init: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      properties: {},
      ctor: function ctor() {
        !cc.sys.isBrowser || cc.sys.os != cc.sys.OS_IOS && cc.sys.os != cc.sys.OS_ANDROID || (config.isWeb = true);
        cc.log("------init\u521d\u59cb\u5316------------");
        window.wsft = window.wsft || {};
    ...
    cc.Class({
      ...
      setSmallGameOver: function setSmallGameOver(data) {
        ...
        if (data.liuju) {
          ...
          for (var i = 0; i < seats.length; ++i) {
            var mySeat = seats[i];
            this.liuJuCard[i] = mySeat.handCard[mySeat.handCard.length - 1];
            mySeat.handCard.pop();
          }
          this.setWinCloseLiuJu(3);
        } else users[wsft.gameData.mySeatIndex - 1].rs > 0 ? this.setWinCloseLiuJu(0) : 0 == users[wsft.gameData.mySeatIndex - 1].rs ? this.setWinCloseLiuJu(1) : this.setWinCloseLiuJu(2);
      ...
      setPlayerMj: function setPlayerMj(data) {
        ...
        for (var i = 0; i < seats.length; ++i) {
          ...
          for (var h in data.hu) if (i == data.hu[h].seetIndex - 1) {
            ...
            hupai.getChildByName("label").active = true;
            huNode.active = true;
          }
          if (data.liuju) {
            console.log("this.liuJuCard,===", this.liuJuCard);
            this.setSpriteFrameByMJ("B_", _hupai, this.liuJuCard[i].num, this.liuJuCard[i].type);
            hupai.getChildByName("label").active = false;
            huNode.active = false;
          }
